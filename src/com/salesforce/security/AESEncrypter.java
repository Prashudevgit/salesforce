...
public class AESEncrypter {
	...
	public static String encrypt(String Data) throws Exception {
		...
		try {
			Key key  = new SecretKeySpec(SalesForceEnvironment.getKeyValue().getBytes("UTF-8"), ALGO);
			Cipher c = Cipher.getInstance("AES/CBC/PKCS5PADDING");
			c.init(Cipher.ENCRYPT_MODE, key, new IvParameterSpec(iv));
			byte[] encVal = c.doFinal(Data.getBytes());
			encryptedValue = Base64.getEncoder().encodeToString(encVal);
		} catch (Exception e) {
			SalesForceEnvironment.setComment(1,classname, " Exception in encrytion "+e.getMessage());		
		}
	...
	public static String decrypt(String encryptedData) throws Exception {
		...
		try {
			...
			SalesForceEnvironment.setComment(1,classname, " Exception in decryption "+e.getMessage());		
		}
