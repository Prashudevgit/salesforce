...
import com.salesforce.dao.MySqlPoolableObjectFactory;
...
//		System.out.println("============== starting :" +className + " at "+java.time.LocalTime.now());
		...
		if(dbConnPool == null){ // Only created by first servlet to call
			try {
				...
				SalesForceEnvironment.setComment(3,className,"Inside init method of HttpServletJXGB...dbConnPool formed "+MySqlPoolableObjectFactory.checkConnPoolformed());										
		}catch (Exception e) {
			SalesForceEnvironment.setComment(1,className,"Internal error formed during the ConnectionPool formation "+e.getMessage());
...
/*		try {
	...
	public void destroy() {
		 try {
			 ...
			 SalesForceEnvironment.setComment(3,className,"Connection Pool shutting down..........."+MySqlPoolableObjectFactory.checkConnPoolformed());
			 }
		 	} catch (Exception e) {
		 		SalesForceEnvironment.setComment(1,className, "Problem with shutting of Connection Pool  "+e.getMessage());			
			}
