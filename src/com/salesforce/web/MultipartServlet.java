...
public class MultipartServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			SalesForceEnvironment.setComment(3,classname," Inside multi ");
			...
			SalesForceEnvironment.setComment(3,classname," Exception is  "+e.getMessage()  );
    	 	response.setContentType("text/html");
 			try {
				context.getRequestDispatcher("/"+SalesForceEnvironment.getServletPath()+"?qs=err").forward(request, response);
			} catch (Exception e1) {
